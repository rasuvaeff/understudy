...
а имена named-аргументов — из первого (основного) интерфейса. Статические методы
контракта присутствуют в сгенерированном классе, чтобы он мог реализовать
интерфейс, но их вызов бросает `InvalidCallSpecification`: у статического
вызова нет экземпляра дубля, которому принадлежало бы состояние.
